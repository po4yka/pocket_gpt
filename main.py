...
def main():
    ...
    parser.add_argument('--fetch-pocket', action='store_true',
                        help='Fetch articles from Pocket')
    parser.add_argument('--fetch-content', action='store_true',
                        help='Fetch content for articles')
    parser.add_argument('--process-articles', action='store_true',
                        help='Process articles with OpenAI GPT')
    parser.add_argument('--update-tags', action='store_true',
                        help='Update articles in Pocket with tags')
...
def fetch_content_for_articles(session, content_fetcher):
    ...
    for article in articles:
        ...
        if content:
            ...
            logger.warning(f"No content fetched for article {
                           article.pocket_id}")
...
def process_articles_with_gpt(session, openai_processor):
    ...
    articles = session.query(Article).filter(
        Article.content.isnot(None), Article.summary.is_(None)).all()
    for article in articles:
        if not article.content:
            logger.warning(f"No content for article {
                           article.pocket_id}, skipping.")
            continue
...
def update_pocket_tags(session, pocket_client):
    ...
    for article in articles:
        ...
        if success:
            ...
            logger.error(f"Failed to update tags for article {
                         article.pocket_id}")
